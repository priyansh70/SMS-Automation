...
export class PaymentCardPage extends BasePage {
  ...
  async fillCardData(cardDetails) {
    ...
    if (cardNumber)
      await this.getCardNumberInput().fill(cardNumber, { delay: 20 });
    if (expirationDate)
      await this.getExpirationDateInput().fill(expirationDate);
    if (cvv) await this.getCVVInput().fill(cvv);
    if (nickName) await this.getNickNameInput().fill(nickName);
    if (billingAddress)
      await this.getBillingAddressInput().fill(billingAddress);
  ...
  async enterPastExpirationAndVerify() {
    ...
    await expect(this.getExpirationErrorMessage()).toBeVisible({
      timeout: 3000,
    });
